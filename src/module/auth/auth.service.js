...
export const signup = async (inputs) => {
    ...
    if (checkAccount) { throw conflictRequestError({ message: "email already exists" }) } 
    const account = await userModel.create({ userName, email, password: await generateHash({ plainText: password }), phone: await encrypt(phone), role, age, gender })
     try { await sendEmailOtP({ email, subject: emailEnum.confirmEmail, title: "verifyEmail" }) } catch (error) { console.error("Failed to send OTP email:", error); }
      return account
}
